...
SYS_SER_RECEIVE_TELNUM_WRITE:
	...
	CALL	INIT_DAM_FUNC
	LACL	0XE700|(0xff)
	CALL	DAM_BIOSFUNC	;Set index-0(bit7-New flag)
...
;!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	LAC	VOP_FG
	ANDL	~((1<<12)|(1<<13))
	SAH	VOP_FG		;See it as normal CID
	
	LACL	CMSG_CID
	;CALL	STOR_MSG
	SAH	MSG
	BS	B1,SYS_MSG_NO	;!!!本次循环中立即执行
...
VOX_CHK_ON_1:
	...
	SAH	CONF
        ...
        RET
VOX_CHK_OFF:                    ; VOX off
        LACL    CTMR_CVOX       ; restore 8.0 sec in TMR_VOX
        SAH     TMR_VOX
VOX_CHK_END:
	LACK    0
	RET
