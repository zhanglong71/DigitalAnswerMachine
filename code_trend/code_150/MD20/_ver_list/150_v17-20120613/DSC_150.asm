...
;TMR3;用于ring detect
;PRO_VAR = 
;ANN_FG :	bit0=	1/0---initial/initial complete(PowerOnInit时置位,Init完毕后清除)
;		bit1=	1/0---caller id valid/caller id invalid(收到来电信息时置位,来铃失败/应答退出/应答密码PASS时清除)
...
;		bit(7..4) = RING_CNT
;		bit(11..8)= OGM_ID
;		bit(13,12)= LANGUAGE(0/1/2/3 - English/German/Reserved/Reserved)
;		bit14 = 1/0------BeepVoicePrompt/not
;		bit15 = 1/0------NewMsgBeepPrompt/not
;VOP_FG		bit(14..0) --- reserved
...
;		--------------------------
;GPBD :		bit0 = CPC-DET				I
;		bit1 = CompressRate			I(0/1 - 4.8/9.6kbps-Set ANN_FG,15)
;		bit2 = ring-det				I
;		bit3 = LED 背光				O
;		bit4 = 2-way hook			O
;		bit5 = LED on/off			O
...
;							Contrast--- (byte9)
;							Language--- (byte10)
;telphone - mode working group 6 ==>(0XE606) 存贮日期/时间
;
...
.extern	LOCAL_PROBOOK1	;look up phonebook(look up)
.extern	LOCAL_PROBOOK2	;(look up phonebook) -> add
.extern	LOCAL_PROBOOK3	;(look up phonebook) -> edit
...
.DATA	0X01	0X01	0X07	0X0F	0X18	0X1A	0X18	0X1D
;.DATA	0X01	0X01	0X06	0X0A	0X12	0X15	0X1A	0X1D
;//----------------------------------------------------------------------
