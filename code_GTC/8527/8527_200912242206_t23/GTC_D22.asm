...
;		bit1=	0/1---DAM-enabler/not(要求DAM不能应答)
;		bit2=
;		bit3=
;		------
;		bit4=	
...
;		bit2 --- request update vol(write into flash)
...
.ORG	ADDR_FIRST
	BS	B1,Main
	BS	B1,IntTimer1	;0x02:  1/1.024-ms timer, goto _timerInt_1
...
IntTimer1_RingChk:
	BIT	ANN_FG,1	;DAM disable ?
	BS	TB,IntTimer1_RingChk_END
	CALL	RING_CHK
...
IntTimer1_CPC_OFFLINE:
	LACL	(CPC_DELAY+CPC_TLEN+OSI_TLEN)
	SAH	TMR_CPC
IntTimer1_CPC_END:
;-------belows are for MUTE-----------------------------------------------------
INT_MUTE_DET:	
