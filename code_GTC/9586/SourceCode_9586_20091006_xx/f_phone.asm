...
.endif		;????????????????????????---for debug
	...
	CALL	DAM_BIOSFUNC
;---Set Silence Level
	LACL	CPHO_0XCA0X		;Default = 7
	CALL	DAM_BIOSFUNC
;---Set Silence Delay Period
	LACL	CPHO_0XCBXX		;Default = 3
	CALL	DAM_BIOSFUNC
...
;---	
	...
	CALL	DAM_BIOSFUNC
;---the related de-noise leavel
	LAC	CONF
	ANDK	0X0F
	ADHL	PHOVOLTONOISE_TAB
	CALL	GetOneConst
	ORL	0XCD00
	CALL	DAM_BIOSFUNC
;---
...
.DATA	0X00	0X00	0X00	0X00	0X08	0X08	0X08	0X08	0x08	0X08	0X0A	0X0A	0X0A	0X0A	0X0A	0X0A	
;---------------------------------------
;PHOVOL_TAB:
