...
CRING_WAIT	.EQU	9000	;RING WAIT时间(即一次来电的多次来铃之间的最长时间)
CMIN_RLEN	.EQU	120	;The min. length of valid ring(有效铃流的最小单个脉冲时间长度)
...
;---ring pulse ok
	...
	LAC     RING_ID		;one ring pulse ok
        ANDL    0XF000
        SBHL    0X5000        	;check if the ring pulse number=5 ?
...
RING_HI_CHK_1:
	...
	LAC	BTONE_BUF
	SBHL	CMIN_RLEN	;check if the ring pulse time length > CMIN_RLEN(ms) ? (!!!误差10ms)
	BS	SGN,RING_CHK_END
;---(Condition pluse >=2 and ring-time >CMIN_RLEN is ok)change to the ring off state
	LACL	CRING_IN
	CALL	INT_STOR_MSG
